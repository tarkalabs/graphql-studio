...
import { Connection, IConnection } from "@tarkalabs/pg-db-utils";
...
export class addConnectionCommand {
  ...
  async run() {
    ...
    if (!(await MultiStepInput.run((input: any) => this.setHostName(input, state)))) {
      // command cancelled
      return;
    }
  ...
  async setHostName(input: MultiStepInput, state: Partial<ConnectionState>) {
    state.host = await input.showInputBox({
      ...
      validate: async (value: string) => (!value || !value.trim()) ? 'Hostname is required' : '' 
    });
  ...
  async setUsername(input: MultiStepInput, state: Partial<ConnectionState>) {
    state.user = await input.showInputBox({
      ...
      validate: async (value: string) => (!value || !value.trim()) ? 'Username is required' : ''
    });
  ...
  async setPort(input: MultiStepInput, state: Partial<ConnectionState>) {
    state.port = await input.showInputBox({
      ...
      convert: async (value: string) => Number.parseInt(value)
    });
  ...
  async setSSL(input: MultiStepInput, state: Partial<ConnectionState>) {
    ...
    state.secure = await input.showQuickPick({
      ...
      convert: async (value: any) => value.ssl
    });
  ...
  async setDatabase(input: MultiStepInput, state: Partial<ConnectionState>) {
    ...
    try {
      ...
      const res = await connection.getDatabases(); 
      databases = res.rows.map<DatabaseQuickPickItem>((database: any) => ({label: database.datname, dbname: database.datname}));
    } catch(err) {
      console.error(err);
    } finally {
    ...
    if (databases.length < 1) {
      ...
      do {
        ...
        connection = await new Connection({
          ...
          port: state.port,
          ssl: state.secure,
          database: databaseToTry
        });
        connectionOK = await connection.testConnection();
        await connection.destroy();
        connection = null;
      } while(!connectionOK);
  ...
  async setDisplayName(input: MultiStepInput, state: Partial<ConnectionState>) {
    state.label = await input.showInputBox({
      ...
      validate: async (value: any) => '' // empty error message (no error)
    });
  }
